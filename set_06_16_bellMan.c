...
void coinChange(int coins[], int n, int amount) {
    int count[10] = {0}; // to store count of each coin used
    int i;
    ...
    for (i = 0; i < n; i++) {
        if (count[i] != 0) printf("%d coin(s) of denomination %d\n", count[i], coins[i]);
    }
    if (amount != 0) printf("Remaining amount that cannot be changed: %d\n", amount);
}
int main() {
    int coins[10];
    int n, amount, i;
